 ...
 * @author omar.brugna
 */
class TrackerBroadcast(private val mHost: TrackerBroadcastHost) : BroadcastReceiver() {
    ...
    companion object {
        ...
        fun notifyDataUpdated(host: TrackerBroadcastHost) {
            notify(host, DATA_UPDATED)
        }

        private fun notify(host: TrackerBroadcastHost, broadcastKey: String, identifier: String = Constants.EMPTY, subIdentifier: String = Constants.EMPTY) {
    ...
    private var mListener: TrackerBroadcastListener? = null
    ...
    fun registerForType(@TrackerBroadcastType type: String) {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        @TrackerBroadcastType val type = intent.action!!
        ...
        if (mListener != null) {
            ...
            mListener!!.onBroadcast(context, @TrackerBroadcastType type, sentByMyself, identifier, subIdentifier)
        }
    }

    fun setListener(listener: TrackerBroadcastListener) {
        mListener = listener
    }
}